    ...
    <x-container class="flex justify-center mt-8">
        ...
        <x-profile-sidebar :user="$user" />
          ...
          <div class="w-full md:w-3/4 md:px-4">
                    ...
                    <div class="text-left ml-6 text-sm md:ml-12">
                        ...
                        <div class="space-y-1 mt-3 md:mt-8 md:space-y-2">
                            <div class="flex items-center gap-x-2 md:gap-x-3">
                                <x-icon name="mail" class="text-gray-700" />
                                <p class="text-gray-700 hover:text-blue-600"><a href="mailto:{{ $user->email }}">{{ $user->email }}</a></p>
                            </div>
                            <div class="flex items-center gap-x-2 md:gap-x-3">
                                <x-icon name="telephone" class="text-gray-700" />
                                <p class="text-gray-700 hover:text-blue-600"><a href="tel:{{$user->phone_number}}">{{ $user->phone_number }}</a></p>
                            </div>
                            @if (($address = $user->addresses)->count() > 0)
                            <div class="flex items-center gap-x-2 md:gap-x-3">
                                <x-icon name="location" class="text-gray-700" />
            ...
            <!-- Order History -->
            <div id="history" class="bg-white rounded-lg shadow-md mt-10 py-4 px-4 md:px-6">
                <h2 class="text-base text-gray-700 font-semibold mb-4 text-center md:text-lg md:text-left">Orders History</h2>
                @if ($orders->count() > 0)
                <div class="grid grid-cols-4 items-center justify-items-center border-b border-gray-200 font-semibold text-slate-700 py-2 text-sm md:text-base md:py-4 md:grid-cols-4 lg:grid-cols-5">
                ...
                <a href="{{ route('orders.show', ['order' => $order->order_code]) }}" class="grid grid-cols-4 items-center even:bg-blue-400 even:text-white rounded justify-items-center text-slate-700 py-2 text-sm hover:bg-slate-100 hover:text-black md:text-base md:py-3 md:grid-cols-4 lg:grid-cols-5">
