<x-auth-layout>
    ...
    <x-container class="flex flex-col justify-center md:flex-row md:justify-around">
        ...
        <x-card class="text-slate-700 w-full p-6 bg-sky-700 md:p-10 md:basis-2/5">
            <h2 class="text-center text-2xl font-semibold mb-8">Create an vendor / supplier Account</h2>
            <form action="/vendor/register" method="POST">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        <div x-data="{open:false}" class="relative flex items-center justify-between mb-1">
                            ...
                            <x-icon name="info" class="text-white ml-1 inline text-blue-200 hover:text-white" x-on:mouseover="open = true" x-on:mouseleave="open = false" />
                            <div x-show="open" class="absolute top-6 left-5 px-6 py-2 bg-white rounded-lg shadow" x-transition x-cloak x-on:mouseover="open = true" x-on:mouseleave="open = false">
                                <p class="text-gray-700">If you omitted the brand name, it will display as <span class="font-medium">"Local brand"</span></p>
                            </div>
                        </div>
                ...
                <div class="flex flex-col justify-between items-center mt-6 md:flex-row">
                    <button type="submit" class="p-1.5 px-4 text-lg rounded text-white bg-blue-400 w-full hover:bg-blue-500 md:w-fit">Sign up</button>
                    <p class="mt-4 md:mt-0">Already have one? <a href="{{ route('vendor.login') }}" class="text-blue-200 hover:text-white">Login</a></p>
                </div>
            </form>
        </x-card>
    </x-container>
